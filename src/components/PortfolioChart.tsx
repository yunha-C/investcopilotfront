...
export const PortfolioChart: React.FC<PortfolioChartProps> = ({
  ...
  console.log("PortfolioChart received:", {
    allocation,
    holdings,
  ...
  const data =
    holdings && holdings.length > 0
      ? holdings.map((holding, index) => {
          console.log("Processing holding for chart:", holding);
          return {
      ...
      : allocation
      ? allocation.map((item) => {
          console.log("Processing allocation for chart:", item);
          return {
            name: item.name,
            value: item.percentage,
            color: item.color,
            // For allocation items, add index for color consistency
            ...(item.name !== "Cash" && { ticker: item.name }),
          };
        })
      : [];
