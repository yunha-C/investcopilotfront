...
export const InsightAnalysis: React.FC = () => {
  const { portfolio, setCurrentStep, activePortfolio, loadUserPortfolios } =
    useInvestmentStore();
    ...
    const analyzeInsight = async () => {
      ...
      try {
        if (activePortfolio) {
          ...
          const apiResponse =
            await portfolioService.getMarketInsightRecommendations(
              activePortfolio.id,
          ...
          if (apiResponse.url_insights.failure) {
            ...
            setAnalysisResult({
              title:
                apiResponse.url_insights.title || "Analysis Not Applicable",
              summary: apiResponse.url_insights.description,
              impact:
                "This content is not suitable for portfolio analysis. Please provide a URL with financial market news, economic analysis, or investment-related content.",
              confidence: 0,
            ...
            const apiAnalysis = {
              title:
                apiResponse.url_insights.title ||
                `Market Analysis: ${new URL(urlToAnalyze).hostname}`,
              summary: apiResponse.url_insights.description,
              impact: apiResponse.url_insights.description,
              confidence:
                apiResponse.trading_actions &&
                apiResponse.trading_actions.length > 0
                  ? 85
                  : 0,
              recommendation:
                apiResponse.trading_actions &&
              ...
              reasoning:
                ...
                apiResponse.trading_actions.length > 0
                  ? `Based on the analysis, we recommend ${
                      apiResponse.trading_actions.length
                    } trading action${
                      apiResponse.trading_actions.length > 1 ? "s" : ""
                    }.`
                  : "No specific trading actions recommended at this time.",
              riskImpact: "Calculated based on trading actions",
              returnImpact:
                "Expected to improve returns based on market analysis",
              url: urlToAnalyze,
  ...
  const handleExecuteTrades = async () => {
    console.log(
      "💰 handleExecuteTrades called, isExecutingTrades:",
      isExecutingTrades
    );
    ...
    try {
      ...
      if (analysisResult.fullApiResponse) {
        ...
        await portfolioService.executeMarketInsightRecommendations(
          activePortfolio.id,
          analysisResult.fullApiResponse
        );
              ...
              {isAnalyzing ? (
                <div className="text-center py-8">
                  ...
                  <p className="text-body-medium text-neutral-600 dark:text-dark-text-secondary mb-8">
                    Our AI is processing the content and evaluating potential
                    portfolio impacts...
                  </p>
                ...
                <div className="space-y-8">
                    ...
                    <div className="flex items-start gap-4 mb-4">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex items-center gap-4 mb-3">
                          <span
                            className={`text-body-small font-medium px-2 py-1 rounded-full ${
                              analysisResult.isValidUrl === false
                                ...
                                : "bg-neutral-200 dark:bg-gray-700 text-neutral-700 dark:text-gray-300"
                            }`}
                          >
                            {analysisResult.isValidUrl === false
                              ? "Not Applicable"
                  ...
                  {/* Trading Actions */}
                  {analysisResult.tradingActions &&
                    analysisResult.tradingActions.length > 0 && (
                      <div className="bg-white dark:bg-dark-surface-secondary border border-neutral-200 dark:border-gray-600 rounded-lg p-6">
                        ...
                        <div className="space-y-3">
                          {analysisResult.tradingActions.map(
                            (action: any, index: number) => (
                              ...
                              >
                                <div className="flex items-center gap-3">
                                  <span
                                    className={`px-3 py-1 rounded-full text-body-small font-medium ${
                                      action.action === "BUY"
                                        ? "bg-positive/20 text-positive"
                                        : "bg-warning/20 text-warning"
                                    }`}
                  ...
                  {/* Suggested Changes */}
                  {analysisResult.suggestedChanges &&
                    analysisResult.suggestedChanges.length > 0 && (
                      <div className="bg-white dark:bg-dark-surface-secondary border border-neutral-200 dark:border-gray-600 rounded-lg p-6">
                        ...
                        <div className="grid md:grid-cols-2 gap-6">
                          <div>
                            <h5 className="text-label-large font-medium text-neutral-800 dark:text-dark-text-primary mb-3">
                              Current Allocation
                            </h5>
                            <PortfolioChart
                          ...
                          <div>
                            ...
                            <div className="space-y-3">
                              {analysisResult.suggestedChanges.map(
                                (change: any, index: number) => (
                                  <div
                                    key={index}
                                    className="flex items-center justify-between p-3 bg-neutral-100 dark:bg-gray-700 rounded-lg"
                                  >
                                    <div className="flex items-center gap-3">
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-body-medium text-neutral-600 dark:text-dark-text-secondary mb-6">
                    We couldn't analyze the provided content. Please try again
